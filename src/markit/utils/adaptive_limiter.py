...
class AdaptiveRateLimiter:
    ...
    def __init__(
        ...
        # Pending slot reductions: release() will "swallow" permits until this is 0
    ...
    def release(self) -> None:
        ...
        If there are pending reductions from a concurrency decrease,
        this release will be "swallowed" to shrink the effective capacity.
        """
        if self._pending_reductions > 0:
            # Swallow this release to reduce effective capacity
    ...
    async def _increase_concurrency(self) -> None:
        ...
        if new > old:
            ...
            # Add slots to existing semaphore by releasing extra permits
            # This avoids deadlock from replacing the semaphore object
            slots_to_add = new - old
    ...
    async def _decrease_concurrency(self) -> None:
        ...
        Uses lazy shrinking: sets pending_reductions so future release()
        calls will be swallowed instead of releasing back to semaphore.
        """
        old = self._current_concurrency
        new = max(
        ...
        if new < old:
            ...
            # Schedule lazy reduction: future releases will be swallowed
            slots_to_remove = old - new
            self._pending_reductions += slots_to_remove
            ...
            log.warning(
                ...
                pending_reductions=self._pending_reductions,
            )
