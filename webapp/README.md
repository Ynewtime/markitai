...
The web UI for `markitai serve`: Vite + React 19 + TypeScript + plain CSS (no utility framework; the sheet carries its own preflight reset).
...
Keep TypeScript on 6.x as long as `typescript-eslint` requires its compiler
API. After a dependency update, run `bun run test`, `bun run lint`,
...
`scripts/sync_webapp_static.sh --check` fails when the committed package
assets are stale. Once installed, `markitai serve` serves the bundled
`serve/static/` directory; during development it falls back to this repo's
`webapp/dist/`.
...
`BUILTIN_PRESET_OPTIONS` in that module is only a fallback, used while
capabilities load or when the server is an older one.
...
- LLM off, or plain mode, suspends alt/description analysis but keeps the
  user's choices, and re-enabling normal LLM processing restores them. With no
  model configured, the LLM-dependent presets are disabled; they never turn on
  silently.

Options that imply each other:

- Screenshot source implies screenshot capture, never LLM. Without LLM, URLs
  return screenshot references in a Markdown wrapper for web preview instead
  of extracted text, and files convert normally. Screenshot source and plain
...
- The Cloudflare URL strategy implies its file backend, matching the CLI, and
  leaving that strategy restores the manual backend choice. An explicit file
  backend replaces both inherited converter flags. Separate notices name the
  selected URL service (Defuddle, Jina or Cloudflare Browser Rendering) and any
  Cloudflare Workers AI file upload; `auto` discloses policy-gated remote
  fallbacks.
...
- While no URLs are typed, `<your-files-or-url-or-url_files>` stands in for
  ...
  the hover and focus help explain the placeholder and the configuration
  assumption.
...
- The options disclosure and the active summary stay available at every
  ...
  long command from pushing controls out of view, and commands wrap at spaces
...
The guard fails the build on any value outside these sets, so adding a step
means changing a token first. Every exemption is declared in
...
same guard fails when the preflight reset (`box-sizing`/`margin` on `*`) goes
missing. Dark tokens have one definition, shared by the `prefers-color-scheme`
block and `[data-theme="dark"]`. `--font-sans` carries a CJK fallback chain
because the UI ships a Chinese dictionary.
