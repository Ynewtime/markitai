# Markitai Performance Optimization Phase 3 - Implementation Spec
...
Based on `markitai-perf-optimization-3.md` analysis, this spec defines three optimization phases:
...
~/.markitai/cache.db         ← Global cache (shared across projects)
.markitai/cache.db           ← Project cache (project-specific)
```
...
| `cli.py` | Add `markitai cache` subcommand (`clear`, `stats`) |
...
# Clear global cache
markitai cache clear --global

# Clear all caches
markitai cache clear --all
```
...
### Configuration Changes (markitai.json)
...
- [x] Add `markitai cache` CLI commands to `cli.py`
...
- [x] Update `markitai.json` with unified configuration
- [x] Update unit tests
