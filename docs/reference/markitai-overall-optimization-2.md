# Markitai Overall Optimization Plan (v2)
...
- `packages/markitai/src/markitai/cli.py` 既负责参数解析，也负责转换 pipeline，且单文件/批处理逻辑存在大量重复。
...
**建议模块结构**（示例，不要求一次到位）：
- `packages/markitai/src/markitai/workflow/__init__.py`
- `packages/markitai/src/markitai/workflow/single.py`
- `packages/markitai/src/markitai/workflow/batch.py`
- `packages/markitai/src/markitai/workflow/reporting.py`
- `packages/markitai/src/markitai/workflow/assets_desc.py`

**拆分原则**：
- CLI（click）：只做参数/配置/日志初始化 + 调用 workflow
- workflow：
  - 接收 `MarkitaiConfig`、`runtime`、`converter`/`processor` 等依赖
...
- 可选持久化缓存（默认关闭）：
  - `output/.markitai-cache/image_analysis.json`
...
- 使用 `markitai.security.sanitize_error_message()` 对外输出。
...
### 9.1 `packages/markitai/src/markitai/llm.py`
...
### 9.2 `packages/markitai/src/markitai/cli.py`
...
- 功能不变：
  - `markitai ./input -o ./output --preset rich --resume` 行为一致
