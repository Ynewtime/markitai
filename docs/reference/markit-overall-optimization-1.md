# Markit Overall Optimization Report (v1)
...
### 3.1 CLI 技术栈描述不一致
- 现状：`docs/spec.md` 架构图写 Typer，但实现为 Click（`packages/markit/src/markit/cli.py`）。
- 影响：文档自相矛盾，误导扩展与测试说明。
...
- 文档与 Schema 已同步：`docs/spec.md`、`packages/markit/src/markit/config.schema.json`、`packages/markit/src/markit/config.py`、`packages/markit/tests/SKILL.md`、相关测试同步更新。
