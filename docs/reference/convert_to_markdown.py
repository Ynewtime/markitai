...
LLM 配置（可选，用于格式优化和图片分析）:
    ...
    - LLM_CHUNK_SIZE: 大文件分块阈值（默认 48000 字符）
...
- YAML Frontmatter 包含以下字段：
  - title: 文件标题（从内容识别，如果包含特殊字符如冒号，请使用双引号包裹，例如 title: "Title: Subtitle"）
  - processed: 当前处理日期 (YYYY-MM-DD)
...
def create_llm_client():
    ...
    try:
        ...
        print("⚠️ 未安装 openai 包，LLM 功能不可用")
    ...
    if api_base:
        ...
        if api_key:
            ...
            print(f"✅ 已启用 {provider} LLM: {model}")
            return client, model
    ...
    if gemini_key:
        ...
        print(f"✅ 已启用 Google Gemini LLM: {model}")
        return client, model
    ...
    if azure_key and azure_endpoint:
        ...
        print(f"✅ 已启用 Azure OpenAI LLM: {model}")
        return client, model
    ...
    if ollama_model:
        ...
        print(f"✅ 已启用 Ollama LLM: {ollama_model}")
...
def optimize_markdown_format(
    ...
    # 准备 Prompt，如果提供了 file_title，则注入到 Prompt 中
    current_prompt = FORMAT_OPTIMIZE_PROMPT
    if file_title:
        # 转义双引号，防止 YAML 语法错误
        safe_title = file_title.replace('"', '\\"')
        # 替换 prompt 中的 title 指令
        current_prompt = current_prompt.replace(
            'title: 文件标题（从内容识别，如果包含特殊字符如冒号，请使用双引号包裹，例如 title: "Title: Subtitle"）',
            f'title: "{safe_title}" (请直接使用此标题)',
        )
    ...
    # 分块处理逻辑
    # 如果文本过长，按段落/章节分块处理，避免输出截断
    # 默认设为 48000 字符（约 20k-30k tokens），适配主流大模型 (64k+ output)
    # 可通过环境变量 LLM_CHUNK_SIZE 覆盖
    DEFAULT_CHUNK_SIZE = 48000

    env_chunk_size = os.environ.get("LLM_CHUNK_SIZE")
    if env_chunk_size:
        try:
            CHUNK_SIZE = int(env_chunk_size)
            print(f"      ⚙️  使用环境变量定义的 CHUNK_SIZE: {CHUNK_SIZE}")
        except ValueError:
    ...
    if len(markdown_text) > CHUNK_SIZE:
        print(
            f"      🔄 文本较长 ({len(markdown_text)} 字符 > {CHUNK_SIZE})，正在分块处理..."
        )
        chunks = []
        current_chunk = []
        current_length = 0
        ...
        # 针对分块的精简提示词（不生成摘要/Frontmatter，只做格式清洗）
        CHUNK_PROMPT = """## 任务
仅对提供的 Markdown 片段进行格式清洗和标准化。

## 核心原则
1. **严禁修改内容**：不要改写、总结或删除任何文本。
2. **严禁生成头部元数据**：不要添加 YAML Frontmatter 或摘要。
3. **保持结构**：仅修复 Markdown 语法（如空行、列表、表格格式），保持原有标题层级。

## 格式规范
- Header 之间保留一个空行
- 列表、引用、表格符合标准 Markdown 语法
- 修正标点与加粗的错误嵌套（如：**文本。** -> **文本**。）
"""
        ...
        for i, chunk in enumerate(chunks):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    opt_chunk = response.choices[0].message.content

                    # 清理代码块包裹
                    if opt_chunk.startswith("```"):
                        lines = opt_chunk.splitlines()
                    ...
                    error_msg = str(e)
                    is_rate_limit = any(
                        x in error_msg for x in ["429", "rate", "quota", "Too Many"]
                    )

                    if is_rate_limit and attempt < max_retries - 1:
                        delay = base_delay * (2**attempt)
                        print(f"        ⏳ API 速率限制，{delay:.0f}秒后重试...")
                        time.sleep(delay)
                    else:
                        print(f"        ⚠️ 片段 {i + 1} 优化失败: {e}")
                        # 失败时保留原片段
    ...
    for attempt in range(max_retries):
        try:
            ...
            prompt_with_date = current_prompt.replace(
                "processed: 当前处理日期 (YYYY-MM-DD)", f"processed: {current_date}"
            )

            response = llm_client.chat.completions.create(
                model=llm_model,
                messages=[
                    {"role": "user", "content": prompt_with_date + markdown_text}
                ],
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    # print(f"        ℹ️ 模型不支持 response_format，尝试普通文本模式")
...
def analyze_images_in_markdown(
    ...
    print(f"      🔍 发现 {len(matches)} 张图片，正在分析...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_match):
            ...
            try:
                ...
                print(f"        ⚠️  [{completed_count}/{len(matches)}] 分析异常: {e}")
...
def convert_old_format(input_file: Path, temp_dir: Path) -> Path | None:
    """转换旧格式文件（.ppt/.doc 等）"""
    ...
    if suffix == ".ppt":
        ...
        if check_ms_office_available():
            print("  🔄 使用 MS Office 转换 .ppt → .pptx ...")
            result = convert_with_ms_office(input_file, temp_dir)
            if result:
                return result
            print("      ⚠️ MS Office 转换失败，尝试 LibreOffice...")

        # 2. 尝试 LibreOffice
        libreoffice = find_libreoffice()
        if libreoffice:
            print("  🔄 使用 LibreOffice 转换 .ppt → .pptx ...")
            result = convert_with_libreoffice(input_file, temp_dir, new_ext)
            if result:
                return result
            print("      ⚠️ LibreOffice 转换失败")

        # 3. 都失败，提示用户手动转换
        print("  ⚠️ 无法自动转换 .ppt 文件")
        print("      📌 建议：用 PowerPoint 打开后另存为 .pptx 格式")
        print("      （手动转换后可保留完整的图片和格式）")
        return None
    ...
    if suffix == ".doc":
        ...
        if check_ms_word_available():
            print("  🔄 使用 MS Word 转换 .doc → .docx ...")
            result = convert_doc_with_ms_word(input_file, temp_dir)
            if result:
                return result
            print("      ⚠️ MS Word 转换失败，尝试 LibreOffice...")

        # 2. 尝试 LibreOffice
        libreoffice = find_libreoffice()
        if libreoffice:
            print("  🔄 使用 LibreOffice 转换 .doc → .docx ...")
            result = convert_with_libreoffice(input_file, temp_dir, new_ext)
            if result:
                return result
            print("      ⚠️ LibreOffice 转换失败")
        ...
        # 3. 都失败，提示用户手动转换
        print("  ⚠️ 无法自动转换 .doc 文件")
        print("      📌 建议：用 Word 打开后另存为 .docx 格式")
        print("      （手动转换后可保留完整的图片和格式）")
        return None
...
def extract_base64_images(markdown_text: str, assets_dir: Path, file_stem: str) -> str:
        ...
        try:
            ...
            if is_emf and libreoffice:
                try:
                    ...
                    print(f"        ⚠️ LibreOffice 转换失败: {e2}")

            print(f"        ⚠️ 格式转换失败 ({original_format}): {e}")
            return None
    ...
    # if img_count > 0 or skip_count > 0:
    #     msg_parts = []
    #     if img_count > 0:
    #         msg_parts.append(f"提取 {img_count} 张")
    #     if skip_count > 0:
    #         msg_parts.append(f"跳过 {skip_count} 张")
    #     print(f"      共 {', '.join(msg_parts)} 图片")
...
def convert_to_markdown(
    ...
    try:
        ...
        markdown_text = extract_base64_images(
            markdown_text, assets_dir, input_file.stem
        )
    ...
    if analyze_attachments and llm_client and llm_model and markdown_text:
        # 如果是在 process_folder 中调用的（通常意味着外层已经并行），
        # 我们可以选择是否启用内层并行。
        # 简单起见，这里假设如果是由 process_file 调用（单文件），可以并行；
        # 但如果是 process_folder 调用的，这里是在 _process_single_file 内部，
        # 为了避免嵌套并行爆炸，建议设为 False。
        # 由于 convert_to_markdown 不知道调用上下文，我们可以通过参数控制。
        # 但目前函数签名没变，我们做一个安全的假设：
        # 如果是单文件处理，效率优先；如果是批量，稳定性优先。
        # 这里我们保守一点，默认不开启内层并发，除非显式修改。
        # 或者，我们可以检查是否已经在主线程中。
        # 更好的方式是给 convert_to_markdown 增加参数 use_inner_concurrency。
        # 鉴于现在是在 _process_single_file 中调用，外层已有并发，这里应该串行。
        # 修正：用户要求宽松的优化，所以这里启用并发。
        # analyze_images_in_markdown 会读取 MAX_IMG_WORKERS (默认5)。
        # 外层 _process_single_file 是并发的 (MAX_WORKERS 默认10)。
        # 总并发 = 10 * 5 = 50，这是用户接受的“最宽松”配置。
        markdown_text = analyze_images_in_markdown(
            markdown_text, assets_dir, llm_client, llm_model, use_concurrency=True
        )
    ...
    if optimize and llm_client and llm_model and markdown_text:
        ...
        markdown_text = optimize_markdown_format(
            markdown_text, llm_client, llm_model, file_title=input_file.stem
        )
...
def _process_single_file(
    ...
    output_root: Path,
    temp_dir: Path,
    # md_with_llm: MarkItDown | None,  # 移除
    # md_without_llm: MarkItDown,      # 移除
    llm_client,
    llm_model: str | None,
    optimize: bool,
    ...
    # 注意：mkdir 可能存在竞争，虽然 exist_ok=True 通常是线程安全的，但在某些文件系统上可能有问题
    # 这里我们假设它是安全的
    ...
    if file.suffix.lower() in OLD_FORMATS:
        ...
        with office_lock:
            working_file = convert_old_format(file, temp_dir)
    ...
    if convert_to_markdown(
        ...
        print(f"  ✅ {file.name} -> output/{output_rel}")
        return 1, 0, 0
    else:
...
def process_folder(
    ...
    if output_dir:
        ...
        # 默认：在输入文件夹的父目录下创建一个 'output' 文件夹
        # 例如: 输入 docs/data -> 输出 docs/output
    ...
    # 递归查找支持的文件（排除 output 目录）
    all_files = []
    for f in folder.rglob("*"):
        if f.is_file() and f.suffix.lower() in SUPPORTED_FORMATS:
            # 排除 output 目录中的文件（防止递归处理输出文件）
            try:
                f.relative_to(output_root)
                continue  # 文件在 output 目录中，跳过
            except ValueError:
                pass  # 文件不在 output 目录中
            all_files.append(f)

    if not all_files:
        print(f"⚠️ 在 {folder} 中没有找到可转换的文件")
    ...
    # 从环境变量获取并发数，默认为 10 (优化后的默认值)
    # 注意：如果包含大量需要 Office 转换的文件，建议设为 1
    max_workers = int(os.environ.get("MAX_WORKERS", "10"))
    print(f"🚀 启动 {max_workers} 个线程进行并行处理...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_file):
            try:
                ...
                print(f"❌ 处理文件异常 {file_path.name}: {e}")
                fail += 1
    ...
    # 统计
    print("=" * 50)
    print(f"✨ 完成! 成功: {success}")
    if fail:
        print(f"   失败: {fail}")
    if skip:
        print(f"   跳过: {skip} (需要 LibreOffice 或转换失败)")
...
def process_file(
    ...
    Args:
        ...
        optimize: 使用 LLM 优化 Markdown 格式
        analyze_attachments: 使用 LLM 分析文档中提取出的附件图片
        analyze_image_files: 使用 LLM 分析图片文件和 PPTX 中的图片
                            （None 表示自动：图片文件默认 True，其他默认 False）
    """
    ...
    # 自动判断：图片文件默认启用 analyze_image_files
    ...
    # 根据文件类型和参数选择 MarkItDown 实例
    # 只有独立图片文件才使用带 LLM 的实例
    file_ext = input_file.suffix.lower()
    use_llm_for_markitdown = (
        analyze_image_files
        and llm_client
        and file_ext in IMAGE_FORMATS  # 只有图片文件，不包括 PPTX
    )
    ...
    if input_file.suffix.lower() in OLD_FORMATS:
        working_file = convert_old_format(input_file, temp_dir)
        if working_file is None:
            return
    ...
    if convert_to_markdown(
        ...
        print(f"\n✅ 转换成功: {out_dir / (input_file.stem + '.md')}")
    else:
        print("\n❌ 转换失败")
