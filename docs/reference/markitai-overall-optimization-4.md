# Markitai Overall Optimization Report 4
...
- Python version: spec says >=3.13, implementation enforces ==3.13.*. This reduces deploy flexibility and contradicts the spec.
  - Evidence: `packages/markitai/pyproject.toml`, `pyproject.toml`, `docs/spec.md`
- Hash length mismatch: spec says 12 hex digits, implementation uses 6. Short hashes increase collision risk for state/report files.
  - Evidence: `docs/spec.md` vs `packages/markitai/src/markitai/cli.py` `compute_task_hash()`
- Report naming mismatch: spec describes reports under `reports/{dir_name}.report.json`, code generates `reports/markitai.<hash>.report.json`.
  - Evidence: `docs/spec.md` vs `packages/markitai/src/markitai/cli.py` and `packages/markitai/src/markitai/batch.py`
...
- Many error paths swallow exceptions (`except Exception`) and continue, which makes failures silent and hard to debug, especially in batch runs.
  - Evidence: `packages/markitai/src/markitai/llm.py`, `packages/markitai/src/markitai/image.py`, `packages/markitai/src/markitai/cli.py`, `packages/markitai/src/markitai/converter/office.py`, `packages/markitai/src/markitai/converter/legacy.py`
- OCR temp file handling uses `NamedTemporaryFile(delete=False)` and deletes in `finally`, but exceptions can still leak temporary files.
  - Evidence: `packages/markitai/src/markitai/ocr.py` `recognize_bytes()`
...
- Default concurrency is high and uncoordinated: batch (10) + LLM (10) + OCR page thread pool can stack unexpectedly.
  - Evidence: `packages/markitai/src/markitai/constants.py`, `packages/markitai/src/markitai/batch.py`, `packages/markitai/src/markitai/converter/pdf.py`
- LLM image cache is capped at 500MB in-memory; persistent cache is 1GB per layer, enabled by default.
  - Evidence: `packages/markitai/src/markitai/llm.py`, `packages/markitai/src/markitai/constants.py`
- PDF pipeline can render screenshots, extract embedded images, and run OCR in parallel, causing repeated heavy I/O and CPU.
  - Evidence: `packages/markitai/src/markitai/converter/pdf.py`
...
- Default log level is DEBUG; LLM calls log model, token counts, and detailed metadata to file logs.
  - Evidence: `packages/markitai/src/markitai/constants.py`, `packages/markitai/src/markitai/cli.py`, `packages/markitai/src/markitai/llm.py`
- Output dir symlink safety only checks the target path once; nested symlinks inside output are not validated.
  - Evidence: `packages/markitai/src/markitai/security.py`, `packages/markitai/src/markitai/cli.py`
...
- LLM/vision/image handling logic is duplicated between `cli.py` and `SingleFileWorkflow`, increasing divergence risk.
  - Evidence: `packages/markitai/src/markitai/cli.py`, `packages/markitai/src/markitai/workflow/single.py`
- Multiple TODOs in `llm.py` indicate incomplete typing/contract consistency.
  - Evidence: `packages/markitai/src/markitai/llm.py` (TODO markers)
...
- `packages/markitai/pyproject.toml`
- `pyproject.toml`
- `docs/spec.md`
- `packages/markitai/src/markitai/cli.py`
- `packages/markitai/src/markitai/batch.py`
- `packages/markitai/src/markitai/llm.py`
- `packages/markitai/src/markitai/ocr.py`
- `packages/markitai/src/markitai/image.py`
- `packages/markitai/src/markitai/security.py`
- `packages/markitai/src/markitai/converter/pdf.py`
- `packages/markitai/src/markitai/converter/office.py`
- `packages/markitai/src/markitai/converter/legacy.py`
