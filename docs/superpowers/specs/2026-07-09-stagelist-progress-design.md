...
| `OutputManager` | 整体退役删除(计划阶段修正):深查发现 auth 流程只是收参数从不调用(`_login_copilot`/`_login_claude_agent` docstring 明说 "not used",oauth_display 直接用 stderr console),`print`/`erase_all`/spinner 的唯一生产调用者是即将删除的 ProgressReporter;`attempt_login` 及 `_login_*` 的 `output_manager` 死参数一并删除 |
...
- `✓ Downloaded 3 images (1.4s)`;0 张时定格为
  `✓ No images to download (0.3s)`(计划阶段修正:阶段已开启,
  定格比降级为 note 行实现更简单且视觉一致)
- `✓ LLM enhanced (48.3s)`
