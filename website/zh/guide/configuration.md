...
| `MARKITAI_NO_REMOTE_FETCH` | 强制将 `fetch.remote_consent` 设为 `never`（`1`、`true` 或 `yes`） |
...
在 `auto` 链路中，未经同意不会将 URL 发送给第三方提取服务（defuddle.md、Jina、Cloudflare）：

| 设置 | 选项 | 默认值 | 说明 |
|------|------|--------|------|
| `fetch.remote_consent` | `ask`, `always`, `never` | `ask` | `ask`：在交互式终端下每次运行询问一次，否则跳过远程服务、仅使用本地策略；`always`：无需询问直接使用远程服务；`never`：仅使用本地策略 |

`MARKITAI_NO_REMOTE_FETCH=1`（或 `true`/`yes`）会强制覆盖为 `never`，不论配置如何。显式选择远程策略（`-s defuddle`、`-s jina` 或 `-s cloudflare`）视为已同意本次运行。
