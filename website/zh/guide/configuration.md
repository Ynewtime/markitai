...
- 计算公式：
  ...
  2. 如有图片：`timeout *= 1.5`，多张图片时额外加 `(图片数 - 1) * 10秒`
  3. 如有预期输出 token 数：加 `tokens / 4`
  4. 限制在 [60, 600] 秒范围内
...
| 策略 | 说明 |
|------|------|
| `auto` | 自动检测：按优先级尝试策略（defuddle → jina → static → playwright → cloudflare）；SPA 域名会提升 playwright 优先级 |
...
1. **Browser Rendering**（`/markdown` API）用于 URL 转 Markdown
...
2. **API Token** — 进入 [My Profile → API Tokens](https://dash.cloudflare.com/profile/api-tokens)，点击 **Create Token**，选择 *Custom token* 模板，添加以下权限：
   ...
   | Account / Browser Rendering | Edit | `/markdown` URL 渲染 |
