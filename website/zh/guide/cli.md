...
强制使用 Defuddle API 抓取 URL。免费，无需认证，内容清洗效果优秀。
...
强制使用静态 HTTP 抓取和内置 webextract。无需外部 API。
...
::: warning
`--playwright`、`--defuddle`、`--static`、`--jina` 和 `--cloudflare` 是互斥的 URL 抓取策略，同时只能使用一个。
:::

### `--kreuzberg`

强制使用 kreuzberg 转换器处理所有文件格式，覆盖内置转换器。这是一个**文件转换**选项（与 URL 抓取策略正交），可以与 `--playwright`、`--defuddle`、`--static` 或 `--jina` 组合使用。但 `--kreuzberg` 和 `--cloudflare` 互斥，因为两者都会覆盖文件转换行为。
...
markitai cache stats --verbose --limit 50   # 限制显示条目数
```
...
markitai doctor --suggest-extras   # 列出推荐的 pip extras
```
...
本地提供商（Gemini、Copilot、Claude、ChatGPT）的认证辅助工具。
...
```bash
markitai auth gemini status
markitai auth gemini status --json    # JSON 输出
```

### `markitai auth gemini login`

运行 Gemini OAuth 登录并保存 Markitai 管理的配置。

```bash
# 默认：Google One 模式
markitai auth gemini login

# Code Assist 模式并指定项目 ID
