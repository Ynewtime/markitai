...
检查系统健康状态、依赖项和认证状态。这是主要的诊断命令。缺少必需依赖时会以非零状态码退出，便于在脚本和 CI 中使用。

```bash
markitai doctor
markitai doctor --fix     # 目前仅会自动安装 Playwright 浏览器；其余缺失项只会打印安装提示
markitai doctor --json    # JSON 输出
markitai doctor --suggest-extras   # 输出适合 `uv tool install "markitai[...]"` 的逗号分隔 extras 列表——始终包含 browser/extra-fetch/kreuzberg/svg/heif，检测到对应 CLI 时还会加上 claude-agent/copilot
```
...
::: tip
`LLM` 分组标题及各检查项的消息文本目前未做本地化，无论 CLI 语言如何都会显示为英文（包括最终的总结行）。`LLM API` 行里的 `API provider(s): ...` 仅在配置了真正的远程 API 模型（非 `claude-agent/`/`copilot/`）时才会出现。
:::
