...
- **No more billed-but-unused reasoning**: `claude-agent/` requests now explicitly disable extended thinking, and `chatgpt/` requests set reachatgpt/gpt-5.4-minine` with low verbosity — markitai's calls are one-shot extraction/cleaning tasks, and the providers' defaults could silently spend a large fraction of each response on reasoning tokens the pipeline never reads
