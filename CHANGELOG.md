...
- **Faster PDFs.** Pages of long PDFs, and of PDFs converted at the same time, are extracted in worker processes that each run the layout model on one thread, with the same output. On an 18-core Mac a 300-page PDF converts 4.1× faster, a batch of 40 PDFs 4.1× faster and a 240-file mixed batch 3.3× faster; each page takes about a third of the CPU time it did, and each worker holds about 450 MB. The hidden-text and scanned-page checks run while the pages are extracted. Ctrl-C stops the workers at once instead of letting the current document finish, and no worker outlives the process that started it, a stopped or killed `markitai serve` included.
...
- **The web workspace converts like the CLI and stays responsive.** A job takes up to 1000 items (was 50), so a folder drop converts in one job like a CLI directory batch; with the PDF workers and pipelined LLM steps, 240 mixed files finish 3.2× faster and 30 documents with LLM enhancement 1.7× faster than in 1.1.0. While a job runs, the API keeps answering in a few milliseconds (it could stall for hundreds): PDF page checks run in the extraction workers, converter backends and the LLM stack load off the event loop, provider login checks run on a thread, and LiteLLM is preloaded at startup when a model is configured. The UI's hashed assets are cached by the browser, API and asset responses are gzip-compressed, and shutting down no longer loads LiteLLM just to close it.
