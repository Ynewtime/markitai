...
- **Web extraction: defuddle main-content selection parity**: generic root selection (`webextract/scoring.py`) is now a port of defuddle's `findMainContent` — an ordered entry-point selector list (`#post`, `.post-content`, …, `article`, `main`, `body`) with position-weighted base scores, the full `scoreElement` signal set (paragraphs, commas, image-density penalty, date/author indicators, footnote presence, nested-table penalty, table-cell bonus, multiplicative link density), deepest-child preference with listing-page detection, block-element score fallback, and old-style table-layout detection. Canonicalized embed links (`iframe` → `<a>`) are now marked and protected from scoring removal, and orphaned "Table of Contents" headings left behind by mobile-style pruning are cleaned up
