...
- **LLM enhancement**: format cleaning, frontmatter metadata, and vision analysis of embedded images, all through [litellm](https://github.com/BerriAI/litellm), so any provider works (OpenAI, Anthropic, Gemini, local CLIs, and more)
...
**Guided installer** (recommended). It installs Python and uv if they are
missing, lets you pick extras and the Playwright browser, and offers a mirror
when the default index is unreachable. Bilingual (EN/中文).
...
**Already have Python 3.11–3.13?** Install the package on its own and run the
two setup steps yourself:
...
model up from the environment; you don't need a config file:
...
The [MCP guide](https://markitai.dev/guide/mcp) covers other clients, LLM
...
[Why Markitai](https://markitai.dev/guide/comparison).
