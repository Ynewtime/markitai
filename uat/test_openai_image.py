#!/usr/bin/env python3
"""UAT: Test OpenAI image analysis with native JSON mode.

OpenAI supports native JSON mode via response_format parameter.

Usage:
    export OPENAI_API_KEY=your_key
    uv run python uat/test_openai_image.py [image_path]
"""
...
async def test_openai(image_path: Path):
    """Test OpenAI image analysis with JSON mode."""
    ...
    print("=" * 60)
    print("UAT: OpenAI Image Analysis")
    print("Provider: OpenAI GPT-4o (with native JSON mode)")
...
if __name__ == "__main__":
    ...
    if not image_path.exists():
        ...
        sys.exit(1)
    success = asyncio.run(test_openai(image_path))
