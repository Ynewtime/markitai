...
class TestBuildFrontmatterConsistency:
    ...
    def test_language_included_when_provided(self) -> None:
        """LLM-detected language should appear in frontmatter dict."""
        result = build_frontmatter_dict(
            source="doc.pdf",
            description="A doc about Python",
            language="en",
        )
        assert result["language"] == "en"

    def test_language_omitted_when_none(self) -> None:
        """Language should not appear in output when not detected."""
        result = build_frontmatter_dict(
            source="doc.pdf",
            description="A doc about Python",
            language=None,
    ...
    def test_language_placed_after_canonical_fields(self) -> None:
        """Language field should appear after markitai_processed."""
        result = build_frontmatter_dict(
            source="doc.pdf",
            description="A doc",
            tags=["test"],
        ...
        keys = list(result.keys())
        assert keys.index("markitai_processed") < keys.index("language")

    def test_extra_meta_language_still_filtered(self) -> None:
        """HTML meta language should still be blocked even when LLM language is accepted."""
        result = build_frontmatter_dict(
            source="doc.pdf",
            description="A doc",
            language="zh",
            extra_meta={"language": "en-us"},
        )
        # LLM-detected language wins, not HTML meta
