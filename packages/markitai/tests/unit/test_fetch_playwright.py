...
class TestBrowserInstalledWithRealCheck:
    """Tests for browser installation check with mocked sync_playwright."""

    def test_handles_sync_playwright_exception(self):
        """Handles exception during sync_playwright initialization."""
        from markitai.fetch_playwright import (
            clear_browser_cache,
            is_playwright_browser_installed,
        )
        ...
        with (
            ...
            patch(
                ...
                side_effect=Exception("Sync playwright error"),
            ),
        ):
            result = is_playwright_browser_installed(use_cache=False)
            assert result is False

    def test_handles_browser_launch_exception(self):
        """Handles exception during browser launch."""
        from markitai.fetch_playwright import (
            clear_browser_cache,
            is_playwright_browser_installed,
        )
        ...
        mock_playwright.chromium.launch.side_effect = Exception("Browser not installed")
