...
def show_device_code(
    ...
    *,
    ...
    """
    c = console or _get_stderr_console()
    c.print("  Sign in with ChatGPT using device code:")
    c.print(f"    Visit: [link={url}]{url}[/link]")
    c.print(f"    Code: [bold cyan]{code}[/]")
    c.print(
        "    [dim]Device codes are a common phishing target. Never share this code.[/]"
    )
