...
- 按需加载可选后端，为 DOCX/XLSX/PPTX/CSV/Notebook 使用专用读取路径，减少重复 DOM 扫描和全文字符串拆分，降低转换启动开销。
- 复杂 Office 内容、公式、图片、元数据及调用方配置的适配器保留原有处理与回退路径。PDF 提取算法和模型配置不变。
- 新增可复现的性能与完整输出审计，网页 URL、Office/PDF、批处理等权评估；记录负载的综合提升约 6 倍，不代表所有输入都能达到该倍数。

### 修复

- 公网 URL 在 Fake-IP DNS 环境下，经远程授权与独立公网 DNS 校验后可使用远程抽取，继续拦截私有地址。
- 完整短文与中日韩文本不再无故回退浏览器；失效基于旧检测逻辑学习的 SPA 缓存。
- 原生网页提取保留 callout 正文、Hacker News 讨论内容，并修正隐藏内容的重试行为。
