...
- **PDF 转换大幅提速。** 长 PDF 以及同时转换的多个 PDF 改由工作进程按页提取，每个进程单线程运行版面模型，输出与原来完全一致。在 18 核的 Mac 上，300 页的 PDF 提速 4.1×，40 个 PDF 的批处理提速 4.1×，240 个文件的混合批处理提速 3.3×；每页所用的 CPU 时间约为原来的三分之一，每个工作进程约占 450 MB 内存。隐藏文字检查和扫描页检查在提取页面的同时进行。按 Ctrl-C 会立刻停止工作进程，不再等当前文档转完；工作进程也不会比启动它的进程活得更久，`markitai serve` 被停止或被强制结束时同样如此。
...
- **网页工作台与 CLI 行为一致，转换期间保持响应。** 每个任务最多 1000 个条目（原为 50），拖入文件夹时和 CLI 的目录批处理一样在一个任务里全部转换；借助 PDF 工作进程和流水线化的 LLM 步骤，240 个混合文件比 1.1.0 快 3.2 倍，30 个启用 LLM 增强的文档快 1.7 倍。任务运行期间 API 仍在几毫秒内响应（以前可能卡住几百毫秒）：PDF 的页面检查在提取进程里完成，转换器依赖和 LLM 组件在事件循环之外加载，登录状态检查放到线程里执行，配置了模型时启动即预加载 LiteLLM。界面中带哈希的静态资源由浏览器缓存，API 和静态资源响应启用 gzip 压缩，关闭服务时不再为了清理而加载 LiteLLM。
